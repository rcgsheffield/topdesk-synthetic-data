...
def generate_topdesk_incident_data(num_records=100):
    """
    ...
    statuses = ["firstLine", "secondLine", "partial", "resolved", "closed", "onHold"]
    ...
    for i in range(num_records):
        ...
        record = {
            ...
            "action": (
                fake.text(max_nb_chars=150) if status in ["resolved", "closed"] else ""
            ),
            # Additional fields
            "duration": duration,
            "costs": round(random.uniform(0, 500), 2) if random.random() > 0.8 else 0,
...
def generate_topdesk_person_data(num_records=50):
    """
    ...
    for i in range(num_records):
        ...
        record = {
            ...
            "startDate": fake.date_between(start_date="-5y", end_date="today").strftime(
                "%Y-%m-%d"
            ),
            "endDate": (
...
def generate_topdesk_asset_data(num_records=75):
    """
    ...
    brands = ["Dell", "HP", "Lenovo", "Apple", "Microsoft", "Cisco", "Canon", "Samsung"]
